...
消息队列可以在任务与任务、任务与中断之间传递消息，队列可以保存有限个具有确定长度的数据单元。队列可保存的最大单元数目被称为队列的长度，在队列创建时需要指定其长度和每个单元（队列项或消息）的大小。通常情况下，队列被作为 FIFO（先进先出）使用，即数据由队尾写入，从队首读出。当然由队列首写入也是可能的



![img](https://img-blog.csdnimg.cn/20210407164849340.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0NodWFuZ2tlX0FuZHk=,size_16,color_FFFFFF,t_70#pic_center)
...
![img](https://img-blog.csdnimg.cn/20210407170209665.jpg?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0NodWFuZ2tlX0FuZHk=,size_16,color_FFFFFF,t_70#pic_center)



队列的结构体 `Queue_t` 如下示，该结构体在 queue.c 文件中定义：
...
在使用队列之前必须先创建队列，有动态和静态创建队列两种方法。以下四个函数原型在 queue.c 文件中定义
...
动态创建队列最终会调用 `xQueueGenericCreate()` 函数，下面来分析该源码（静态创建类似，不做分析）
...
队列初始化函数 `prvInitialiseNewQueue()` 源码分析:
...
队列复位函数 `xQueueGenericReset()` 源码分析
...
任务级入队函数最终都是调用 `xQueueGenericSend() `函数，下面来分析该函数源码
...
中断级入队函数最终都是调用 xQueueGenericSendFromISR() 函数，下面来分析该函数源码:
...
任务级出队函数最终都是调用 `xQueueGenericReceive() `函数，出队与入队的源码流程差不多，在此不再赘述
